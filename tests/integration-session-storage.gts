...
import { sessionStorage, clearSessionStorageCache } from '#src/index.ts';
...
export default class TestComponent extends Component<TestComponentSignature> {
  @sessionStorage foo?: string;

  @sessionStorage bar?: string;
...
module(
  'Integration | Component | test-component (sessionStorage)',
  function (hooks) {
    ...
    test('it works inside a component', async function (assert) {
      ...
      assert.dom().hasText('');
      assert.equal(JSON.parse(window.sessionStorage.getItem('foo')!), null);

      await click('button');
      assert.dom().hasText('foo');
      assert.equal(JSON.parse(window.sessionStorage.getItem('foo')!), 'foo');
    });
