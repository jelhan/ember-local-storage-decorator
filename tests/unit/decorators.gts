    ...
    module(`new Unit | Decorator | @${name}`, function (hooks) {
      ...
      module('getter', function () {
        test(`picks up simple value from ${name}`, function (assert) {
          storage.setItem('foo', JSON.stringify('baz'));
        ...
        test(`supports custom ${name} key`, function (assert) {
          storage.setItem('bar', JSON.stringify('baz'));
      ...
      module('setter', function () {
        ...
        test(`persists simple value in ${name}`, function (assert) {
          ...
          klass.foo = 'baz';
          assert.equal(JSON.parse(storage.getItem('foo')!), 'baz');
        });
        ...
        test(`persists complex value in ${name}`, function (assert) {
          ...
          klass.foo = value;
          assert.deepEqual(JSON.parse(storage.getItem('foo')!), value);
