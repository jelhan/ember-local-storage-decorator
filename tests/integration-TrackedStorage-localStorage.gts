...
module('Integration | TrackedStorage (localStorage)', function (hooks) {
  ...
  test('works with complex objects', async function (assert) {
    ...
    class TestComponent extends Component {
      ...
      <template>
        ...
        <button type="button" {{on "click" this.updateUser}} data-test-btn></button>
      </template>
    }
  ...
  test('multiple keys are independent', async function (assert) {
    ...
    class TestComponent extends Component {
      ...
      <template>
        ...
        <button type="button" {{on "click" this.updateFoo}} data-test-foo-btn></button>
        <button type="button" {{on "click" this.updateBar}} data-test-bar-btn></button>
      </template>
    }
  ...
  test('removeItem works', async function (assert) {
    ...
    class TestComponent extends Component {
      ...
      <template>
        ...
        <button type="button" {{on "click" this.remove}} data-test-btn></button>
      </template>
    }
  ...
  test('clear removes all items', async function (assert) {
    ...
    class TestComponent extends Component {
      ...
      <template>
        ...
        <button type="button" {{on "click" this.clearAll}} data-test-btn></button>
      </template>
    }
  ...
  test('persists to underlying storage', function (assert) {
    ...
    assert.equal(
      JSON.parse(window.localStorage.getItem(`${DEFAULT_PREFIX}:persisted`)!),
      'test value',
    );
  });

  test('loads existing values from storage', function (assert) {
    window.localStorage.setItem(`${DEFAULT_PREFIX}:existing`, JSON.stringify('preexisting'));
    
    const newStorage = new TrackedStorage(window.localStorage);
  ...
  test('setting value to null removes it from storage', function (assert) {
    ...
    assert.ok(window.localStorage.getItem(`${DEFAULT_PREFIX}:test`));

    storage.setItem('test', null);
    assert.equal(storage.getItem('test'), null);
    assert.equal(window.localStorage.getItem(`${DEFAULT_PREFIX}:test`), null);
  });
